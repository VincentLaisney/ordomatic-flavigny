...
def dict_tempo_create(current_year, even_year, year_letter):
    ...
    for i in range(2):
        ...
        new_dim_adv["num_day"] = "\\textbf{" + \
            str(new_dim_adv_date.day) + "} & " + new_dim_adv["symbols"] + " \\textbf{\\textsc{Dominica}}"
        new_dim_adv["header"] = " \\textbf{\\textsc{" + f_roman_numbers(
    ...
    dim_3_adv["num_day"] = "\\textbf{" + str(dim_3_adv_date.day) + "} & " + dim_3_adv["symbols"] + " \\textbf{\\textsc{Dominica}}"
    dim_3_adv["header"] = " \\textbf{\\textsc{III Adventus}, Gaudete} - de ea - \\textit{Viol.}"
    dim_3_adv["symbols"] = ""
    ...
    dim_4_adv["num_day"] = "\\textbf{" + str(dim_4_adv_date.day) + "} & " + dim_4_adv["symbols"] + " \\textbf{\\textsc{Dominica}}"
    dim_4_adv["header"] = " \\textbf{\\textsc{IV Adventus}} - de ea - \\textit{Viol.}"
    dim_4_adv["symbols"] = ""
    ...
    dim_4_adv["II_vesp"] = "\\item I Vesperæ sollemnitatis sequentis.\n\\item Completorium omittitur ab his qui sollemnem Vigiliam et Missam in nocte intersunt." if dim_4_adv_date.day == 24 else ""
    ...
    if vigile_noel_date.weekday() != 6:
        ...
        vigile_noel["II_vesp"] = "\\item I Vesperæ sollemnitatis sequentis.\n\\item Completorium omittitur ab his qui sollemnem Vigiliam et Missam in nocte intersunt."
    ...
    noel["num_day"] = "\\textbf{" + str(noel_date.day) + "} & " + \
        f_transf_weekday(noel_date.weekday()) + ("\\textbf{\\textsc{Dominica}}" if noel_date.weekday() == 6 else "")
    noel["header"] = " - " + \
    ...
    dim_2_ap_noel["num_day"] = "\\textbf{" + \
        str(dim_2_ap_noel_date.day) + "} & " + dim_2_ap_noel["symbols"] + " \\textbf{\\textsc{Dominica}}"
    dim_2_ap_noel["header"] = " \\textbf{\\textsc{II post Nativitatem}} - de ea - \\textit{Alb}."
    dim_2_ap_noel["symbols"] = ""
    ...
    hebdo_psalt_vigiles = "; psalmi anni {}".format(
        hebdo_psalterii_inv[even_year])
    bapteme["num_day"] = "\\textbf{" + str(bapteme_date.day) + "} & \\textbf{\\textsc{Dominica}}"
    ...
    for i in range(4):
        ...
        new_dim_careme["num_day"] = "\\textbf{" + \
            str(new_dim_careme_date.day) + "} & " + new_dim_careme["symbols"] + " \\textbf{\\textsc{Dominica}}"
        new_dim_careme["symbols"] = ""
        dim_laetare = ", Lætare}" if i == 3 else "}"
        new_dim_careme["header"] = " \\textbf{\\textsc{" + f_roman_numbers(
        ...
        for j in range(6):
            ...
            if (new_day_careme_date - cendres_date).days < 11: # 1ere semaine
                if new_day_careme_date.weekday() in [2,4]:
                    new_day_careme["body"] = "\n\\item \\textit{In ML (Viol.): Quatuor Temporum Quadragesimæ.}"
                elif new_day_careme_date.weekday() == 5:
                    new_day_careme["body"] = "\n\\item \\textit{In ML (Viol.): Quatuor Temporum Quadragesimæ (forma Missæ brevior).}"
    ...
    passion["num_day"] = "\\textbf{" + str(passion_date.day) + "} & " + passion["symbols"] + " \\textbf{\\textsc{Dominica}}"
    passion["header"] = " \\textbf{\\textsc{V in Quadragesima}} (I Passionis) - de ea - \\textit{Viol.}"
    passion["symbols"] = ""
    ...
    rameaux["num_day"] = "\\textbf{" + str(rameaux_date.day) + "} & " + rameaux["symbols"] + " \\textbf{\\textsc{Dominica}}"
    rameaux["header"] = " \\textbf{\\textsc{in Palmis de Passione Domini}} - de ea - \\textit{Rub.}"
    rameaux["symbols"] = ""
    rameaux[
    ...
    # On ferme l'accolade.
    samedi_saint[
        "body"] = "\n\\item In Officio: altare nudum sit, cum cruce tamen et quatuor candelabris accensis. In Vigiliis et Laudibus, candelabrum triangulare cum quindecim cereis adhibetur.\n\\item Ad mensam: ß \\textit{Principes}.\n\\item Vesperæ celebrantur hora consueta.\n\\item Completorium necnon et Vigiliæ omittuntur ab his qui Vigiliam paschalem intersunt.}\n\\bigskip"
    samedi_saint[
    ...
    for i in range(5):
        ...
        if i == 0:
            ...
            dim_ap_paques[
                "body"] = "\n\\item Ad Vigilias: in I Nocturno sumuntur lectiones 3 et 4 (quorum titulus est: \\textit{De Actibus Apostolórum}) cum ¶ 4."
    ...
    dim_7_paques["num_day"] = "\\textbf{" + str(dim_7_paques_date.day) + "} & " + dim_7_paques["symbols"] + " \\textbf{\\textsc{Dominica}}"
    dim_7_paques["header"] = " \\textbf{\\textsc{VII Paschæ}} (seu Dominica post Ascensionem vel infra octavam Ascensionis) - de ea - \\textit{Alb}."
    dim_7_paques["symbols"] = ""
    ...
    lectures_pentecote = {
        'A': "\n\\item[Dom. A] Act \\textbf{2}, 1-11 / 1 Cor \\textbf{12}, 3b-7.12-13 / Io \\textbf{20}, 19-23",
    ...
    lectiones_fd = {"A": "Deut \\textbf{8}, 2-3.14b-16a / 1 Cor \\textbf{10}, 16-17 / Io \\textbf{6}, 51-59",
                    "B": "Ex \\textbf{24}, 3-8 / Hebr \\textbf{9}, 11-15 / Mc \\textbf{14}, 12-16.22-26", "C": "Gen \\textbf{14}, 18-20 / 1 Cor \\textbf{11}, 23-26 / Lc \\textbf{9}, 11b-17"}
        ...
        if i != 0:
            if i == 1:  # Trinité.
                ...
                trinite["preface_feries"] = "\n\\item In feriis: præfatio communis " + \
                    num_pref_fer + " , nisi aliter notetur."
                trinite_lectiones = {"A": "Ex \\textbf{34}, 4b-6.8-9 / 2 Cor \\textbf{13}, 11-13 / Io \\textbf{3}, 16-18",
